...
export const MangaMainPageItem = () => {
   const MangaMainPageStore = useSelector((state: IRootState) => state.manga.mainPageMangaArr);

   return(
      <>
      {MangaMainPageStore?.map((manga: MangaType) => (
            <Link style={{ textDecoration: "none" }}  key={manga.id} title={manga.attributes?.canonicalTitle} to={`manga/${manga.id}`}>
